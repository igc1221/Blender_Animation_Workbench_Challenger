...
class BAW_OT_figure_fit_scale_axis(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if self.axis == "XYZ":
            radial = mouse - self._pivot_2d
            if abs(radial.x) >= abs(radial.y):
                self._screen_axis = Vector(
                    (1.0 if radial.x >= 0.0 else -1.0, 0.0)
                )
            else:
                self._screen_axis = Vector(
                    (0.0, 1.0 if radial.y >= 0.0 else -1.0)
                )
        else:
            axis = self._axis_world()
            if axis is None:
        ...
        try:
            self._gesture = begin_fit_scale_gesture(
                context,
                part_id=self._part_id,
                axes=self.axis,
            )
    ...
    def _cancel_gesture(self, context) -> None:
        ...
        if gesture is None:
            return
        try:
            cancel_fit_scale_gesture(context, gesture)
        except FitSemanticSessionError:
            pass
    ...
    def modal(self, context, event):
        if event.type == "MOUSEMOVE":
            ...
            desired = max(0.02, exp(pixel_delta / 260.0))
            try:
                receipt = apply_fit_scale_preview(
                    context,
                    gesture=self._gesture,
                    factor=desired,
                )
                ...
                self.report({"WARNING"}, str(exc))
                return {"CANCELLED"}
            self._applied_scale = desired
            trace_event(
                "INPUT",
                "FIT_F3_SCALE_PREVIEW",
                context=context,
                part_id=self._part_id,
                axis=self.axis,
        ...
        if event.type == "LEFTMOUSE" and event.value == "RELEASE":
            try:
                changed = commit_fit_scale_gesture(context, self._gesture)
            except FitSemanticSessionError as exc:
                self._cancel_gesture(context)
                trace_event(
                    "ERROR",
            ...
            trace_event(
                ...
                part_id=self._part_id,
                axis=self.axis,
                factor=float(self._applied_scale),
                fit_revision=(None if session is None else int(session.revision)),
                preview_serial=(
                    None if session is None else int(session.preview_serial)
                ),
                changed=changed,
            )
            return {"FINISHED"}
...
class BAW_GGT_global_transform(bpy.types.GizmoGroup):
    ...
    def draw_prepare(self, context):
        ...
        if route == "NATIVE" and hasattr(context.space_data, "show_gizmo_tool"):
            ...
            context.space_data.show_gizmo_tool = False
        pivot, axes = _pivot_axes(context, route, mode)
        if pivot is None or axes is None:
            return
